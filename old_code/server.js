...
app.post('/generate-poem-v2', async (req, res) => {
  ...
  try {
    ...
    if (assistantMessage) {
      ...
      const scriptText = assistantMessage.content[0].text.value;

      // Insert the new record into the "comedy" table
      ...
      if (error) {
        console.error('Error inserting into "comedy" table:', error);
        res.status(500).json({ error: 'Failed to insert into "comedy" table' });
      } else {
        console.log('Inserted into "comedy" table:', data);
      }
    } else {
